...
begin
  ...
  try
     BancoDados := Configuracoes.ReadString('XML', 'DATABASE', DriverName);
     DriverName := Configuracoes.ReadString('XML', 'DriverName', DriverName);
     UserName   := Configuracoes.ReadString('XML', 'UserName',   UserName);
     PassWord   := Decoder64.DecodeString(Configuracoes.ReadString('XML', 'PASSWORD', ''));
  finally
    Configuracoes.Free;
    Decoder64.Free;
  end;
  ...
  try
    ...
    MessageDlg('Erro ao conectar o Banco de dados XML ' + BancoDados,mtInformation,[mbOK],0);
  end;
end;
