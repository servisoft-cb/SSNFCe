...
begin
  ...
  try
    ...
    begin
      ...
      if (FormTelaAutenticaUsuario.ModalResult = MrOK) and (FormTelaAutenticaUsuario.EditSenha.Text <> EmptyStr) then
      ...
      xSql := 'select ID, USUARIO from PARAMETROS_USUARIO where SENHA_CUPOM = :SENHA';
      sds.SQLConnection := dmDatabase.scoDados;
      sds.NoMetadata    := True;
      sds.GetMetadata   := False;
      sds.CommandText   := xSql;
      sds.ParamByName('SENHA').AsString := Password;
      sds.Open;
      if sds.IsEmpty then
      begin
      ...
      begin
        UserName := sds.fieldbyname('USUARIO').Value;
        SenhaOK  := True;
        ContUser := 3;
      end;
    end;
    ...
    begin
      AutenticaUsuario                   := 'S';
      InfoRetorno.CodUsuarioAutenticado  := sds.FieldByName('ID').AsInteger;
      InfoRetorno.NomeUsuarioAutenticado := sds.FieldByName('USUARIO').AsString;
