...
type
  TdmCupomFiscal = class(TDataModule)
    ...
    sdsCupomParametrosMOSTRAR_PRODUTO_ADI: TStringField;
    cdsCupomParametrosMOSTRAR_PRODUTO_ADI: TStringField;
    ...
    cdsProduto_ConsumoADICIONAL: TStringField;
    ...
    cdsProduto_AdicionalID_MATERIAL: TIntegerField;
    cdsProduto_AdicionalQTD_CONSUMO: TFloatField;
    cdsProduto_AdicionalNOME_MATERIAL: TStringField;
    ...
    mAdicional: TClientDataSet;
    mAdicionalID_Produto: TIntegerField;
    mAdicionalNome: TStringField;
    mAdicionalAdicional: TStringField;
    dsmAdicional: TDataSource;
    sdsCupomFiscal_Item_Sem: TSQLDataSet;
    sdsCupomFiscal_Item_SemID: TIntegerField;
    sdsCupomFiscal_Item_SemITEM: TIntegerField;
    sdsCupomFiscal_Item_SemID_PRODUTO: TIntegerField;
    sdsCupomFiscal_Item_SemVALOR: TFloatField;
    cdsCupomFiscal_Item_Sem: TClientDataSet;
    dsmCupom_Itens: TDataSource;
    cdsCupom_ItenssdsCupomFiscal_Item_Sem: TDataSetField;
    dsCupomFiscal_Item_Sem: TDataSource;
    cdsCupomFiscal_Item_SemID: TIntegerField;
    cdsCupomFiscal_Item_SemITEM: TIntegerField;
    cdsCupomFiscal_Item_SemID_PRODUTO: TIntegerField;
    cdsCupomFiscal_Item_SemVALOR: TFloatField;
    mAdicionalValor: TFloatField;
...
procedure TdmCupomFiscal.prc_Gravar_Rateio_Recibo_Troca(ID_Cupom: Integer);
var
  aCalculoRateio : TCalculaRateio;
begin
  prcLocalizar(ID_Cupom);
  aCalculoRateio := TCalculaRateio.Create;
  try
    aCalculoRateio.ValorRateio := cdsCupomFiscalVLR_RECIBO_TROCA.AsFloat;
    aCalculoRateio.ValorTotalProdutos := cdsCupomFiscalVLR_PRODUTOS.AsFloat;
    cdsCupom_Itens.First;
    while not cdscupom_itens.Eof do
    begin
      cdsCupom_Itens.Edit;
      aCalculoRateio.ValorProduto := cdsCupom_ItensVLR_TOTAL.AsFloat;
      cdsCupom_ItensVALOR_RATEIO_RECIBO_TROCA.AsFloat := aCalculoRateio.CalcularRateio;
      cdsCupom_Itens.Post;
      cdsCupom_Itens.ApplyUpdates(0);
      cdsCupom_Itens.Next;
    end;
    ...
    begin
      cdsCupom_Itens.Last;
      cdsCupom_Itens.Edit;
      cdsCupom_ItensVALOR_RATEIO_RECIBO_TROCA.AsFloat := cdsCupom_ItensVALOR_RATEIO_RECIBO_TROCA.AsFloat + (cdsCupomFiscalVLR_RECIBO_TROCA.AsFloat - aCalculoRateio.ValorDiferenca);
      cdsCupom_Itens.Post;
      cdsCupom_Itens.ApplyUpdates(0);
    end;
  finally
...
var
  ffrmSel_Produto_Preco: TfrmSel_Produto_Preco;
  vItem: Integer;
  i: Integer;
  vTexto2: String;
begin
...
//    Exit;
  ...
  sqlConsulta.SQL.Add('VT.VLR_VENDA1, VT.VLR_VENDA2, VT.VLR_VENDA3, VT.VLR_VENDA4, PRO.USA_COR, PRO.OBS, PRO.USA_PRECO_COR, ');
  sqlConsulta.SQL.Add('VT.NOME_TAB1, VT.NOME_TAB2, VT.NOME_TAB3, VT.NOME_TAB4, ');
  ...
  if cdsCupomFiscalFILIAL.AsInteger > 0 then
    sqlConsulta.ParamByName('FILIAL').AsInteger := cdsCupomFiscalFILIAL.AsInteger
  else
  begin
    cdsFilial.First;
    sqlConsulta.ParamByName('FILIAL').AsInteger := cdsFilialID.AsInteger;
  end;
  ...
  if (sqlConsulta.FieldByName('VLR_VENDA1').AsFloat <= 0) and
     ...
     (sqlConsulta.FieldByName('VLR_VENDA3').AsFloat <= 0) and
     (sqlConsulta.FieldByName('VLR_VENDA4').AsFloat <= 0) then
  begin
    Result := False;
    Exit;
  end;

  ffrmSel_Produto_Preco := TfrmSel_Produto_Preco.Create(Self);
  try

    vItem := 1;
    ffrmSel_Produto_Preco.mPreco.Insert;
    ffrmSel_Produto_Preco.mPrecoItem.AsInteger := vItem;
    ffrmSel_Produto_Preco.mPrecoNome.AsString  := 'Cadastro';
    ffrmSel_Produto_Preco.mPrecoPreco.AsFloat  := StrToFloat(FormatFloat('0.00###',sqlConsulta.FieldByName('PRECO_VENDA').AsFloat));
    ffrmSel_Produto_Preco.mPreco.Post;
    vItem := vItem + 1;
    ...
    begin
      ffrmSel_Produto_Preco.mPreco.Insert;
      ffrmSel_Produto_Preco.mPrecoItem.AsInteger := vItem;
      ffrmSel_Produto_Preco.mPrecoNome.AsString  := 'Preço Promocional';
      ffrmSel_Produto_Preco.mPrecoPreco.AsFloat := StrToFloat(FormatFloat('0.00###',sqlConsulta.FieldByName('PRECO_PROMOCAO').AsFloat));
      ffrmSel_Produto_Preco.mPreco.Post;
      vItem := vItem + 1;
    end;
    ...
    for i := 1 to 4 do
    begin
      if StrToFloat(FormatFloat('0.00###',sqlConsulta.FieldByName('VLR_VENDA'+IntToStr(i)).AsFloat)) > 0 then
      begin
        ffrmSel_Produto_Preco.mPreco.Insert;
        ffrmSel_Produto_Preco.mPrecoItem.AsInteger := vItem;
        ffrmSel_Produto_Preco.mPrecoNome.AsString  := sqlConsulta.FieldByName('NOME_TAB'+IntToStr(i)).AsString;
        ffrmSel_Produto_Preco.mPrecoPreco.AsFloat := StrToFloat(FormatFloat('0.00###',sqlConsulta.FieldByName('VLR_VENDA'+IntToStr(i)).AsFloat));
        ffrmSel_Produto_Preco.mPreco.Post;
        vItem := vItem + 1;
      end;
    end;
    if vID_TabPreco_CV > 0 then
    begin
      vTexto2 := SQLLocate('TAB_PRECO','ID','NOME',IntToStr(vID_TabPreco_CV));
      if vTexto2 <> '' then
        ffrmSel_Produto_Preco.mPreco.Locate('Nome',vTexto2,[loCaseInsensitive]);
    end;
    ffrmSel_Produto_Preco.ShowModal;

  finally
    FreeAndNil(ffrmSel_Produto_Preco);
  end;

end;

procedure TdmCupomFiscal.prc_Gravar_Log(ID: Integer; Tipo: String);
var
  sds: TSQLDataSet;
  vGravar : Boolean;
begin
